...
//	private int[][] dataVolume;
	...
	// DataNode 数 (仮)
	private static final int DATA_NODE_COUNT = 4;
	...
	protected JobInProgress(JobID jobid, JobConf conf, JobTracker tracker) 
		...
		// dataVolume の初期化
		//dataVolume = new int[DATA_NODE_COUNT][conf.getNumReduceTasks()];
		dataVolumes = new TreeMap<String, int[]>();
		try {
	...
	public synchronized void updateTaskStatusAndTask(TaskInProgress tip, 
			TaskStatus status, MapTask task) {
		...
		if (change) {
			...
			if (state == TaskStatus.State.SUCCEEDED) {
				...
				tip.setSuccessEventNumber(taskCompletionEventTracker);
				if (task.isMapTask()) {
					// 成功したタスクのパーティション毎のデータ量を格納
					int[] taskDataVolume = task.getDataVolume();
					int[] oldDataVolume = dataVolumes.get(taskTracker.getTrackerName());
					for (int i = 0; i < taskDataVolume.length; i++) {
						oldDataVolume[i] += taskDataVolume[i];
					}
					dataVolumes.put(taskTracker.getTrackerName(), oldDataVolume);
					show(dataVolumes);
				}
			} else if (state == TaskStatus.State.COMMIT_PENDING) {
				// If it is the first attempt reporting COMMIT_PENDING
	...
	private void arrayShow(String key, int[] array) {
